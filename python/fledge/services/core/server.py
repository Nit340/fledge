...
# --- Real-time Data Storage (Added for South Plugin Communication) ---
...
# --- CORS Middleware (Added for South Plugin Communication) ---
from functools import wraps # Ensure this import is present or add it near other imports
...
# ... (rest of the file, including the Server class) ...
class Server:
    """ Fledge core server.
  ...
  # ... (inside the Server class, around the _make_core_app method) ...
...
def _make_core_app(cls):
    ...
    :rtype: web.Application
    """
    # --- Add CORS Middleware (Added for South Plugin Communication) ---
    cors_mw = cors_middleware_factory() # Create an instance of our CORS middleware
    # -------------------------------------------------------------------

    # --- Apply CORS Middleware and existing middleware ---
    # Include cors_mw in the middlewares list.
    # The order matters: cors_mw is typically outermost for responses.
    # error_middleware is usually innermost.
    # aiohttp applies middlewares in reverse order of the list for the *request* path,
    # and normal order for the *response* path.
    app = web.Application(
    ...
    # --- Register the NEW Real-time Data Routes (Added for South Plugin Communication) ---
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(management_api_url, data=None, headers=headers) as resp:
                ...
                try:
                    ...
                    raise
# ... (other methods in the Server class) ...

# --- Real-time Data Endpoint Handlers (Added for South Plugin Communication) ---
@classmethod
...
# ... (rest of the class definition, like _make_app, _make_core_app, start, etc.) ...
    @classmethod
